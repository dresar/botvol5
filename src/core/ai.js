...
User yang sedang chat: ${userName}\n\n`;
    ...
    getAIHelp() {
        return `🤖 *PANDUAN AI CHAT*\n\n` +
               ...
               `💡 *TIPS:*\n` +
               `• AI akan ingat ${this.maxHistory} pesan terakhir\n` +
    ...
    async handleCommand(msg, args) {
        try {
            ...
            const senderName = chat.isGroup ? (await msg.getContact()).pushname || 'User' : 'User';
            
            // Process AI commands
            if (args.length === 0) {
                // Show AI help if no arguments
                await msg.reply(this.getAIHelp());
                return;
            }
            ...
            if (subCommand === 'stats' || subCommand === 'statistik') {
                const stats = await this.getAIStats(sender);
                await msg.reply(stats);
                return;
            }
            
            if (subCommand === 'globalstats') {
                const globalStats = await this.getGlobalAIStats();
                await msg.reply(globalStats);
                return;
            }
            
            if (subCommand === 'clear' || subCommand === 'hapus') {
                const result = this.clearMemory(sender);
                await msg.reply(result);
                return;
            }
            
            if (subCommand === 'clearall') {
                const result = await this.clearAllData(sender);
                await msg.reply(result);
                return;
            }
            
            if (subCommand === 'loadhistory' || subCommand === 'load') {
                const result = await this.loadHistoryFromDatabase(sender);
                await msg.reply(result);
                return;
            }
            
            // If no specific command, treat the entire args as a message to AI
            ...
            const response = await this.generateResponse(sender, userMessage, senderName);
            ...
            await msg.reply('❌ Terjadi kesalahan saat memproses perintah AI. Silakan coba lagi nanti.');
        }
    }
}
