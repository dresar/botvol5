...
` +
               `🔄 *TEMPLATE RESPONS:*
` +
               `• /addresponse [keyword]|[response]|[type] - Tambah template
` +
               `• /listresponses - Lihat daftar template
` +
               `• /removeresponse [id] - Hapus template
` +
               `• /updateresponse [id]|[keyword]|[response]|[type] - Update
...
` +
               `• /messagelogs - Lihat log pesan terbaru
` +
               `• /clearmessagelogs - Hapus semua log pesan
` +
               `• /clearmessagelogsdays [days] - Hapus log pesan lebih lama dari X hari
...
` +
    ...
    async handleCommand(args, msg) {
        ...
        switch (command) {
            case 'addadmin':
                if (args.length < 2) return '❌ Format: /addadmin [userId]';
                return await this.addAdmin(args[1], userId);
                
            case 'removeadmin':
                if (args.length < 2) return '❌ Format: /removeadmin [userId]';
            ...
            case 'setapikey':
                if (args.length < 2) return '❌ Format: /setapikey [apiKey]';
            ...
            case 'addapikey':
                if (args.length < 4) return '❌ Format: /addapikey [provider] [key] [name] [dailyLimit] [monthlyLimit]';
                const provider = args[1].toLowerCase();
                const apiKey = args[2];
                const name = args[3];
                const dailyLimit = args.length > 4 ? parseInt(args[4]) : 1000;
                const monthlyLimit = args.length > 5 ? parseInt(args[5]) : 30000;
                return await this.addAPIKey(provider, apiKey, name, userId, dailyLimit, monthlyLimit);
                
            case 'removeapikey2':
                if (args.length < 2) return '❌ Format: /removeapikey2 [keyId]';
                return await this.removeAPIKeyById(args[1], userId);
                
            case 'listapikeys':
                const providerFilter = args.length > 1 ? args[1].toLowerCase() : null;
                return await this.listAPIKeys(providerFilter);
            ...
            case 'messagelogs':
                ...
                return await this.getMessageLogs(page);
                
            case 'clearmessagelogs':
                return await this.clearAllMessageLogs(userId);
                
            case 'clearmessagelogsdays':
                if (args.length < 2) return '❌ Format: /clearmessagelogsdays [days]';
                const days = parseInt(args[1]);
                return await this.clearMessageLogsByDate(days, userId);
            ...
            case 'addresponse':
                if (args.length < 2) return '❌ Format: /addresponse [keyword]|[response]|[type]';
                const responseParts = args.slice(1).join(' ').split('|');
                if (responseParts.length < 2) return '❌ Format salah! Gunakan: /addresponse [keyword]|[response]|[type]';
                const responseType = responseParts.length > 2 ? responseParts[2].toLowerCase() : 'all';
                return await this.addResponseTemplate(responseParts[0], responseParts[1], responseType, userId);
                
            case 'listresponses':
                return await this.listResponseTemplates();
                
            case 'removeresponse':
                if (args.length < 2) return '❌ Format: /removeresponse [id]';
                return await this.removeResponseTemplate(args[1], userId);
                
            case 'updateresponse':
                if (args.length < 2) return '❌ Format: /updateresponse [id]|[keyword]|[response]|[type]';
                const updateParts = args.slice(1).join(' ').split('|');
                if (updateParts.length < 3) return '❌ Format salah! Gunakan: /updateresponse [id]|[keyword]|[response]|[type]';
                const updateType = updateParts.length > 3 ? updateParts[3].toLowerCase() : 'all';
                return await this.updateResponseTemplate(updateParts[0], updateParts[1], updateParts[2], updateType, userId);
    ...
    async addResponseTemplate(keyword, response, type, addedBy) {
        try {
            if (!keyword || !response) {
                return '❌ Keyword dan response harus diisi!';
            }
            
            let isPrivateOnly = false;
            ...
            switch (type) {
                ...
                case 'all':
                default:
                    // Both flags false means available for both private and group
                    break;
            }
            
            const id = await this.db.addResponseTemplate(keyword, response, isPrivateOnly, isGroupOnly, addedBy);
            
            return `✅ *TEMPLATE RESPONS DITAMBAHKAN*\n\n` +
                   `🔑 ID: ${id}\n` +
                   `🔍 Keyword: ${keyword}\n` +
                   `💬 Response: ${response.substring(0, 50)}${response.length > 50 ? '...' : ''}\n` +
                   `🔄 Tipe: ${isPrivateOnly ? 'Private Chat' : (isGroupOnly ? 'Group Chat' : 'Semua Chat')}\n\n` +
                   `💡 Template akan otomatis digunakan saat ada pesan yang cocok.`;
        } catch (error) {
            // Removed console.error for faster execution
            return '❌ Gagal menambahkan template respons!';
        }
    }
    
    async listResponseTemplates() {
        try {
            const templates = await this.db.getResponseTemplates();
            
            if (templates.length === 0) {
                return `📋 *DAFTAR TEMPLATE RESPONS*\n\n❌ Tidak ada template respons yang tersedia.\n\n💡 Tambahkan dengan: /addresponse [keyword]|[response]|[type]`;
            }
            
            let response = `📋 *DAFTAR TEMPLATE RESPONS*\n\n`;
            
            templates.forEach((template, index) => {
                const type = template.is_private_only ? 'Private Chat' : (template.is_group_only ? 'Group Chat' : 'Semua Chat');
                response += `${index + 1}. 🆔 ID: ${template.id}\n`;
                response += `   🔍 Keyword: ${template.keyword}\n`;
                response += `   💬 Response: ${template.response.substring(0, 50)}${template.response.length > 50 ? '...' : ''}\n`;
                response += `   🔄 Tipe: ${type}\n`;
                response += `   ⏰ Dibuat: ${new Date(template.created_at).toLocaleDateString('id-ID')}\n\n`;
            });
            
            response += `📊 Total templates: ${templates.length}\n`;
            response += `💡 Hapus template: /removeresponse [id]\n`;
            response += `✏️ Update template: /updateresponse [id]|[keyword]|[response]|[type]`;
            
            return response;
        } catch (error) {
            // Removed console.error for faster execution
            return '❌ Gagal mengambil daftar template respons!';
        }
    }
    
    async removeResponseTemplate(id, removedBy) {
        try {
            const template = await this.db.getResponseTemplateById(id);
            
            if (!template) {
                return `❌ Template dengan ID ${id} tidak ditemukan!`;
            }
            
            await this.db.removeResponseTemplate(id);
            
            return `✅ *TEMPLATE RESPONS DIHAPUS*\n\n` +
                   `🆔 ID: ${id}\n` +
                   `🔍 Keyword: ${template.keyword}\n` +
                   `💬 Response: ${template.response.substring(0, 50)}${template.response.length > 50 ? '...' : ''}\n\n` +
                   `💡 Template telah dihapus dan tidak akan digunakan lagi.`;
        } catch (error) {
            // Removed console.error for faster execution
            return '❌ Gagal menghapus template respons!';
        }
    }
    
    async updateResponseTemplate(id, keyword, response, type, updatedBy) {
        try {
            const template = await this.db.getResponseTemplateById(id);
            ...
            await this.db.updateResponseTemplate(id, keyword, response, isPrivateOnly, isGroupOnly);
            
            return `✅ *TEMPLATE RESPONS DIUPDATE*\n\n` +
                   `🆔 ID: ${id}\n` +
                   `🔍 Keyword Baru: ${keyword}\n` +
                   `💬 Response Baru: ${response.substring(0, 50)}${response.length > 50 ? '...' : ''}\n` +
                   `🔄 Tipe: ${isPrivateOnly ? 'Private Chat' : (isGroupOnly ? 'Group Chat' : 'Semua Chat')}\n\n` +
                   `💡 Template telah diupdate dan akan digunakan untuk pesan yang cocok.`;
        } catch (error) {
            // Removed console.error for faster execution
            return '❌ Gagal mengupdate template respons!';
        }
    }

    // Tambahkan fungsi untuk mengaktifkan koneksi instan
    async enableInstantConnection(enabledBy) {
        if (!this.isAdmin(enabledBy)) {
            return '❌ Hanya admin yang bisa mengaktifkan koneksi instan!'; 
        }
        
        try {
            // Set flag di database
            await this.db.setSetting('instant_connection', 'true');
            
            // Update global setting jika ada
            if (global.botSettings) {
                global.botSettings.instantConnection = true;
            }
            
            return '✅ Koneksi instan berhasil diaktifkan! Bot akan terhubung tanpa pemeriksaan Gemini dan monitoring.\n\n⚠️ Catatan: Fitur ini mengurangi keamanan dan pemantauan bot.';
        } catch (error) {
            return '❌ Gagal mengaktifkan koneksi instan: ' + error.message;
        }
    }
    
    // Tambahkan fungsi untuk menonaktifkan koneksi instan
    async disableInstantConnection(disabledBy) {
        if (!this.isAdmin(disabledBy)) {
            return '❌ Hanya admin yang bisa menonaktifkan koneksi instan!';
        }
        
        try {
            // Set flag di database
            await this.db.setSetting('instant_connection', 'false');
            
            // Update global setting jika ada
            if (global.botSettings) {
                global.botSettings.instantConnection = false;
            }
            
            return '✅ Koneksi instan berhasil dinonaktifkan! Bot akan melakukan pemeriksaan normal saat terhubung.';
        } catch (error) {
            return '❌ Gagal menonaktifkan koneksi instan: ' + error.message;
        }
    }
    
    // Tambahkan fungsi untuk mengecek status koneksi instan
    async checkInstantConnection() {
        try {
            const setting = await this.db.getSetting('instant_connection');
            const isEnabled = setting === 'true';
            
            let response = `🔌 *STATUS KONEKSI INSTAN*\n\n`;
            response += `Status: ${isEnabled ? '✅ Aktif' : '❌ Nonaktif'}\n`;
            response += `\n💡 Koneksi instan memungkinkan bot terhubung tanpa pemeriksaan Gemini dan monitoring.\n`;
            response += `\n⚙️ Perintah:\n`;
            response += `• /enableinstant - Aktifkan koneksi instan\n`;
            response += `• /disableinstant - Nonaktifkan koneksi instan`;
            
            return response;
        } catch (error) {
            return '❌ Gagal mengecek status koneksi instan: ' + error.message;
        }
    }
    ...
    async getMessageLogs(page = 1, limit = 20) {
        try {
            ...
            const logs = await this.db.getMessageLogs(limit, offset);
            const totalCount = await this.db.getMessageCount();
            const totalPages = Math.ceil(totalCount / limit);
            
            if (logs.length === 0) {
                return '📋 *LOG PESAN*\n\n❌ Tidak ada log pesan yang tersedia.';
            }
            
            let response = `📋 *LOG PESAN* (Halaman ${page}/${totalPages})\n\n`;
            
            logs.forEach((log, index) => {
                const messageType = log.type === 'group' ? '👥' : '👤';
                const formattedTime = log.formatted_time || new Date(log.timestamp).toLocaleString('id-ID');
                const content = log.content.length > 30 ? log.content.substring(0, 30) + '...' : log.content;
                
                response += `${index + 1}. ${messageType} ${formattedTime}\n`;
                response += `   👤 User: ${log.user_id}\n`;
                if (log.group_id) response += `   👥 Group: ${log.group_id}\n`;
                response += `   💬 Pesan: ${content}\n\n`;
            });
            
            response += `📊 Total pesan: ${totalCount}\n`;
            response += `📄 Halaman: ${page}/${totalPages}\n`;
            response += `💡 Gunakan /messagelogs [halaman] untuk melihat halaman lain\n`;
            response += `🗑️ Gunakan /clearmessagelogs untuk menghapus semua log pesan`;
            
            return response;
        } catch (error) {
            console.error('Error getting message logs:', error);
            return '❌ Gagal mengambil log pesan!';
        }
    }
    
    // Clear all message logs
    async clearAllMessageLogs(clearedBy) {
        if (!this.isAdmin(clearedBy)) {
            return '❌ Hanya admin yang bisa menghapus log pesan!';
        }
        
        try {
            const totalDeleted = await this.db.clearAllMessageLogs();
            
            return `✅ *LOG PESAN DIHAPUS*\n\n` +
                   `🗑️ ${totalDeleted} pesan telah dihapus dari database\n` +
                   `⏰ Waktu: ${new Date().toLocaleString('id-ID')}\n\n` +
                   `💡 Semua log pesan telah dihapus.`;
        } catch (error) {
            console.error('Error clearing message logs:', error);
            return '❌ Gagal menghapus log pesan!';
        }
    }
    
    // Clear message logs older than X days
    async clearMessageLogsByDate(days, clearedBy) {
        if (!this.isAdmin(clearedBy)) {
            return '❌ Hanya admin yang bisa menghapus log pesan!';
        }
        ...
        try {
            const totalDeleted = await this.db.clearMessageLogsByDate(days);
            
            return `✅ *LOG PESAN DIHAPUS*\n\n` +
                   `🗑️ ${totalDeleted} pesan yang lebih lama dari ${days} hari telah dihapus\n` +
                   `⏰ Waktu: ${new Date().toLocaleString('id-ID')}\n\n` +
                   `💡 Log pesan lama telah dibersihkan.`;
        } catch (error) {
            console.error('Error clearing message logs by date:', error);
            return '❌ Gagal menghapus log pesan!';
        }
    }
}
